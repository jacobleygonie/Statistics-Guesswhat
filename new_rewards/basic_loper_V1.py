...
class BasicLooper(object):
    def __init__(self, config, oracle_wrapper, qgen_wrapper, guesser_wrapper, tokenizer, batch_size):
        ...
        self.pen_spatial = config['loop']['pen_spatial']
        self.avoid_words = config['loop']['avoid_words']
        self.update_reward= config['loop']['update_reward']
    ...
    def process(self, sess, iterator, mode, optimizer=list(), store_games=False):
        ...
        for game_data in tqdm(iterator):
            ...
            if len(optimizer) > 0:
                ...
                final_reward = [int(found_object[i])-nb_badwords[i]*self.pen_spatial for i in range(len(nb_badwords))] # + 0  # +1 if found otherwise 0
