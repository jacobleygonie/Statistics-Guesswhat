...
class FirstQuestionCategory(AbstractPlotter):
    def __init__(self, path, games, logger, suffix):
        super(FirstQuestionCategory, self).__init__(path, self.__class__.__name__, suffix)
                ...
                if(category_used):
                    for obj in game.objects :
                        if (obj.category ==possible_category):                                
                            category_in=True
                person=False
                if (possible_category=="person" or possible_category=="food"):
                    person=True
                if (category_used == True and category_in==True and game.status=="success" and person==False):
                    category_used_in_success+=1
                if (category_used == True and category_in==True and game.status=="failure" and person == False):
                    category_used_in_failure+=1
                if (category_used == True and category_in==True and game.status=="success" and person):
                    category_used_in_success_PERSON+=1
                if (category_used == True and category_in==True and game.status=="failure" and person):
                    category_used_in_failure_PERSON+=1
        ...
        labels = 'used, in, success','used, in, failure','person / food used, in, success','person / food used, in, failure','used, not in, success','used, not in, failure', 'not used, success', 'not_used, failure'
        colors = ['yellowgreen', 'gold', 'lightskyblue','lightcoral','lightgray','peru','w','r']
        plt.title("Is a category used as a word in the first question ? Is this category is in the image ? Success ? ")
        plt.pie([ category_used_in_success,category_used_in_failure,category_used_in_success_PERSON, category_used_in_failure_PERSON, category_used_out_success,category_used_out_failure,no_category_success,no_category_failure], labels=labels, colors=colors,  autopct='%1.1f%%',shadow=True, startangle=90)
